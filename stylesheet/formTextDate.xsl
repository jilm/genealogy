...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template match="date[year and month and day]"
                  mode="#default" priority="3">
        <xsl:value-of
           select="concat(day/text(), '.', month/text(), '.', year/text())" />
    </xsl:template>

    <xsl:template match="date[year and month]"
                  mode="#default" priority="2">
        <xsl:value-of select="concat(month/text(), '.', year/text())" />
    </xsl:template>

    <xsl:template match="date[year and month and day]"
                  mode="long text" priority="3">
